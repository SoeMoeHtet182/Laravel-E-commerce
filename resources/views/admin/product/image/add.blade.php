...
@section('content')
    <div class="row">
        <div class="col">
            <a href="{{ url('/admin/product') }}"><button class="btn btn-dark">Back</button></a>
            <hr>
            <form action="{{ url('/admin/add-product_images/' . $product->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <input type="submit" value='Upload' class='btn btn-primary float-end mt-3'>
            </form>
        </div>
    </div>
@endsection
